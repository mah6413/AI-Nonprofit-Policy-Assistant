...
PIPELINE_MODULE_NAME = os.getenv("RAG_PIPELINE_MODULE", "hybrid_rag_pipeline").strip() or "hybrid_rag_pipeline"


def load_rag_pipeline() -> Any:
    """Load the selected RAG pipeline without overwriting the original module."""
    candidates = [PIPELINE_MODULE_NAME]
    if "updated_rag_pipeline" not in candidates:
        candidates.append("updated_rag_pipeline")
    ...
    st.info(
        "For Streamlit Cloud, confirm requirements.txt includes openai, python-dotenv, "
        "pypdf, python-docx, python-pptx, openpyxl, and reportlab."
    )
    st.stop()


rag_pipeline = load_rag_pipeline()
CATEGORY_TITLES = rag_pipeline.CATEGORY_TITLES
DOCUMENT_CATALOG = rag_pipeline.DOCUMENT_CATALOG
export_outputs = rag_pipeline.export_outputs
run_nonprofit_assistant = rag_pipeline.run_nonprofit_assistant
