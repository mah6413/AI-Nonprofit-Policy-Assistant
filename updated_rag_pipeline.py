...
def retrieve_relevant_chunks(question: str, chunk_store: List[Dict[str, Any]], client: OpenAI, top_k: int = 6, embedding_model: str = DEFAULT_EMBEDDING_MODEL) -> List[Dict[str, Any]]:
    question_embedding = get_embedding(question, client, embedding_model)
    scored = []
    for chunk in chunk_store:
        if chunk["embedding"] is not None:
            item = {key: chunk[key] for key in ("chunk_id", "document_id", "document_title", "document_type", "source_path", "location", "locator", "char_start", "char_end", "text")}
            item["score"] = cosine_similarity(question_embedding, chunk["embedding"])
            scored.append(item)
    return sorted(scored, key=lambda item: item["score"], reverse=True)[:top_k]


def evaluate_retrieval_quality(retrieved_chunks: List[Dict[str, Any]]) -> str:
    if not retrieved_chunks or retrieved_chunks[0]["score"] < 0.25:
        return "no_relevant_context"
    return "partial" if retrieved_chunks[0]["score"] < 0.55 else "strong"
...
def _context_from_chunks(chunks: List[Dict[str, Any]]) -> str:
    return "\n\n".join(f"[C{index} | {item['document_title']} | {item['location']} | relevance={item['score']:.3f}]\n{item['text']}" for index, item in enumerate(chunks, 1))
...
def answer_question(question: str, chunk_store: List[Dict[str, Any]], client: OpenAI, top_k: int = 6, chat_model: str = DEFAULT_CHAT_MODEL, citation_base_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    if quality == "no_relevant_context":
        return {"answer": "I could not find enough support for that answer in the uploaded document repository.", "citations": [], "confidence": "low", "fallback_used": True}
    ...
    return {"answer": answer, "citations": citations, "confidence": "high" if quality == "strong" else "medium", "fallback_used": False}
...
def generate_grounded_document(user_request: str, document_type: str, chunk_store: List[Dict[str, Any]], client: OpenAI, top_k: int = 8, chat_model: str = DEFAULT_CHAT_MODEL, citation_base_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    quality = evaluate_retrieval_quality(retrieved)
    if quality == "no_relevant_context":
        return {"answer": f"I could not find enough support in the uploaded repository to generate a reliable {spec['title']}.", "document_type": key, "category": spec["category"], "citations": [], "confidence": "low", "fallback_used": True}
...
""".strip()
    response = client.chat.completions.create(model=chat_model, messages=[{"role": "system", "content": BASE_SYSTEM_PROMPT}, {"role": "user", "content": prompt}])
    answer = link_citation_tokens(response.choices[0].message.content, citations)
    return {"answer": answer, "document_type": key, "category": spec["category"], "citations": citations, "confidence": "high" if quality == "strong" else "medium", "fallback_used": False}
